...
class MLPPlotter2D:
    ...
    def plot_interactive(self, w_x_0, w_x_1, w_y_0, w_y_1, w_h_0, w_h_1, b_0, b_1, b_h, activation_function_index):
        ...
        # Create subplots
        fig, axes = pl.subplots(nrows=1, ncols=2, figsize=(12, 6))

        # Plot the first subplot
        axes[0].clear()
        img = axes[0].imshow(np.flipud(output_matrix), interpolation='None', extent=(self.xlim[0], self.xlim[1], self.ylim[0], self.ylim[1]), vmin=-1, vmax=1, cmap='turbo')
        pl.colorbar(img, shrink=0.7, ax=axes[0])

        if len(self.data) > 0:
            axes[0].scatter(x=self.data[self.c1_i,0], y=self.data[self.c1_i,1], c='r', s=100, edgecolors='black')
            axes[0].scatter(x=self.data[self.c2_i,0], y=self.data[self.c2_i,1], c='b', s=100, edgecolors='black')
        axes[0].set_xlabel('x')
        axes[0].set_ylabel('y')
        axes[0].grid()
        axes[0].set_title('Perceptron output')
        if len(self.data) > 0:
            ...
            axes[1].clear()
            axes[1].plot(self.error)
            axes[1].set_xlabel('Iterations')
            axes[1].set_ylabel('MSE')
            axes[1].grid()
            axes[1].set_title('Perceptron error')
    ...
    def init_animation(self):
        if not self.ax_im:
            ...
            pl.grid()
        self.im = self.ax_im.imshow(np.zeros(self.input_x_matrix.shape), interpolation='None', vmin=-1, vmax=1, extent=(self.xlim[0], self.xlim[1], self.ylim[0], self.ylim[1]), cmap="turbo")
        pl.colorbar(self.im, shrink=0.7)
        self.ax_im.scatter(x=self.data[self.c1_i,0], y=self.data[self.c1_i,1], c='r', s=100, edgecolors='black')
        self.ax_im.scatter(x=self.data[self.c2_i,0], y=self.data[self.c2_i,1], c='b', s=100, edgecolors='black')
